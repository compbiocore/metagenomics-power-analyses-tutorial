...
After within-group mean and standard deviations have been computed, we will use those to determine the level of subsampling and number of OTUs needed to produce OTU tables with similar properties as the control group.
...
```{r,within_dist}
jaccard<-vegdist(otu_table[,2:148],method="jaccard",na.rm=TRUE)
...
Once we have determined population parameters for within-group distances, we now use `hashMean` to simulate a list of OTU tables at different levels of subsampling (`rare_levels`) so that we can determine what level of subsampling produces OTU tables with the same mean pairwise distance. Since we are doing an unweighted analysis, we set `sequence_depth`=1, but if weighted then we would need to decide or determine the sequencing depth as well. Once we have determine the level of subsampling, then we will use `hashSD` to simulate a list of OTU tables at different numbers of OTUs at our specific subsampling level to determine what number of OTUs produces OTU tables with the same standard deviation of pairwise distance. These are the additional parameters that will be used to simulate OTU tables under a range of effect sizes. More on the theory behind this and the validation is available in the paper.

The names of `hashMean` are the levels of subsampling with the index number.
...
mean_df <- data.frame(x=as.numeric(names(means)),y=as.numeric(means))
qplot(data=mean_df,x=x,y=y) + geom_smooth(method="loess")
loess <- loess(y~x, data=df)
subsample <- predict(loess,newdata=data.frame(x=m))
...
```{r, bootPower}
bp10<-bootPower(uj, boot_number=10, subject_group_vector=c(10,10),alpha=0.05)
bp25<-bootPower(uj, boot_number=10, subject_group_vector=c(25,25),alpha=0.05)
bp50<-bootPower(uj, boot_number=10, subject_group_vector=c(50,50),alpha=0.05)
